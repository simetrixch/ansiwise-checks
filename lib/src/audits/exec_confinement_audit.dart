/// The suite that drives [ExecConfinement] over one tree, with the counter-probe beside it.
library;
...
/// Runs the exec-confinement audit over [tree], or over the repository this suite sits in.
///
/// WHAT IT STATES: how many Dart files the rule applied to. Empty means the scan decided about
/// nothing, which is the one outcome that reads like a pass and is not one.
void auditExecConfinement({SourceTree? tree}) {
  final ExecConfinement check = ExecConfinement(tree ?? SourceTree.on(repositoryRoot()));
  final int confined = check.confinedFiles.length;
  ...
  test('$confined Dart file(s) of a shipped library were scanned', () {
    expect(
      check.confinedFiles,
      isNotEmpty,
      reason:
