/// The suite that drives [RegistryCompleteness] over one registry and tree, with its counter-probe.
library;
...
///
/// WHAT IT STATES: how many entries were read out of the registry, and how many step classes were
/// found on disk to count them against. A registry read as empty would pass every assertion below
/// without making one, and a tree in which no step class was found leaves the unregistered half
/// measuring nothing.
void auditRegistryCompleteness(Registry registry, {SourceTree? tree}) {
  final SourceTree judged = tree ?? SourceTree.on(repositoryRoot());
  final RegistryReading reading = RegistryReading.of(registry);
  final RegistryCompleteness check = RegistryCompleteness(tree: judged, reading: reading);
  final int entries = reading.entries.length;
  final int predicates = reading.predicates.length;
  final int onDisk = check.classesOnDisk.length;

  test('$entries entry/entries and $predicates predicate(s) were read out of the registry', () {
    expect(
      reading.entries,
      isNotEmpty,
  ...
  test('$onDisk step class(es) were found under $stepsDirectory to count against', () {
    expect(
      check.classesOnDisk,
      isNotEmpty,
      reason:
  ...
  group('counter-probe', () {
    // Both halves of the source verification, over a file this audit writes: the right line must be
    ...
    for (final String shape in <String>[
      ...
      test('$shape is found on disk', () {
        expect(
          ...
          contains(shape),
          reason:
              'a step class this scan cannot see is a step nothing reports as unregistered, and the '
              'unregistered half of this check is then green for having looked past it',
        );
      });
    }
    ...
    test('the same entry pointing at the declaration is left alone', () {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'this check reports every entry, so its silence about the real registry means nothing',
      );
    });

    // The predicate half. A package that registers no predicate exercises none of these lines with
    // its own registry — which is exactly why they are planted: the day it registers one, the rule
    // that judges it has been shown to work rather than assumed to.
...
/// Where the planted step file sits in the tree this audit writes.
const String _plantedPath = '$stepsDirectory/planted.dart';

/// Where the planted predicate file sits in the tree this audit writes.
...
///
/// Line 2 is the declaration; line 1 is a doc comment naming the class as prose. Written as a list
/// of lines for the same reason as [_plantedDeclarations].
...
final RegisteredStep _plantedEntry = _entryAt('$_plantedPath:4');
...
/// scanned by the reversibility audit next door: a line reading `class X extends Step` at column
/// zero would be a declaration as far as that scan is concerned, and the tree holding this file
/// would go red for a class that does not exist.
