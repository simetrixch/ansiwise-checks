/// The suite that drives [Naming] over one tree, with the counter-probe beside it.
library;
...
/// Runs the naming audit over [tree], or over the repository this suite sits in.
///
/// WHAT IT STATES: how many names and how many Dart files it judged. Both are denominators — a run
/// that walked nothing answers "no abolished word found" exactly as a clean tree does.
void auditNaming({SourceTree? tree}) {
  final Naming check = Naming(tree ?? SourceTree.on(repositoryRoot()));
  final int names = check.namesJudged.length;
  final int files = check.dartFilesJudged.length;

  test('$names name(s) were judged', () {
    expect(
      check.namesJudged,
      isNotEmpty,
  ...
  test('$files Dart file(s) were read for sub-command declarations', () {
    expect(
  ...
  group('counter-probe', () {
    ...
    final List<Finding> plantedNames = planted.nameFindings;
    ...
    for (final String path in <String>[
      ...
      test('the planted name $path is reported', () {
        expect(about(plantedNames, path), isNotEmpty, reason: 'the name scan cannot go red');
      });
    }
...
///
/// This file is itself scanned by the check it holds, wherever it is resolved from. Writing the
/// planted names into the source straight after a marker would declare them here, and the check
/// would report itself; assembled this way the marker on each template line yields the interpolation
/// and not a forbidden name.
