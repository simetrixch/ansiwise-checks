...
/// filter is a glob: it has no alternation and `[...]` matches one character, so two things the
/// grammar says cannot be written into `on.push.tags` at all. The first is that a number carries no
/// leading zero — `0|[1-9][0-9]*` is unwritable — and [numbersRefusalFor] is the one place that
/// closes it, because `01.2.3-stable-…` would be published here and then refused by
...
/// what no glob can say — whether a release is a pre-release, and how far the tag may then reach.
///
...
/// version is refused twice for one reason.
...
/// A tag the filter accepts, read into the parts a screen orders and marks it by.
...
final class Releases {
  ...
  /// itself is the one its manifests declare, so [declaredVersion] is what is offered and nothing is
      ...
      return <Proposal>[
        if (declaredVersion != null)
          Proposal(
            ...
            because: 'the first release — the version the packages of this repository declare',
          ),
      ];
    }
      ...
      return <Proposal>[
        Proposal(
          ...
          because:
              ...
              '${channel.riper.map((ReleaseChannel each) => each.name).join(' or ')}',
        ),
      ];
    }
    return <Proposal>[
