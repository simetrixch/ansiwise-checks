...
/// GitHub Release, and no tag for a caller to name in its manifest. The patterns are read out of
/// that file every time tool/release.dart runs, and what a person types is held against them. A
/// grammar spelled a second time in Dart would be a second answer to "may this be released", and the
/// day the workflow's filter was widened or narrowed the two would disagree with nothing in this
...
///
/// IT IS ALSO ASKED ABOUT REFS THAT ARE NOT TAGS OF THIS REPOSITORY. A manifest here names another
/// repository at a `ref:`, and the one grammar covers every release of everything — so the same
/// filter answers whether such a ref is a released tag or a branch somebody is following.
library;

/// The workflow whose `on.push.tags` decides which tags start a release, relative to the repository
/// root — which is the directory ABOVE the package this program lives in.
...
/// a release is cut on a checkout where no package need be resolved. What is walked is three keys
/// and a list of items under them, and a file that does not carry them answers with nothing, which
/// [TagFilter.unreadable] turns into a refusal rather than into a filter that accepts everything.
  ...
  for (final String line in workflow.split('\n')) {
    ...
    if (depth == path.length) {
      ...
      if (content.startsWith('- ')) {
        patterns.add(unquoted(content.substring(2).trim()));
      }
      continue;
    }
    if (indent <= enclosing) {
...
String? _keyOf(String content) {
  ...
  return colon < 0 ? null : unquoted(content.substring(0, colon).trim());
}

/// [text] without the quotes YAML lets a value carry.
String unquoted(String text) {
