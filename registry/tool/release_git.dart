...
///
/// The same split registry/tool/release_manifest.dart makes: what the release program DECIDES is one thing
/// and starting a program on this operating system is another. It is what lets the deciding half be
/// driven by a check — including the half that pushes — on a machine that starts no git and reaches
/// no remote, and what was asked for is then a list of argument lists in the order it was asked.
/// That list is the evidence for the two claims of this program: the screen writes nothing, and a
/// version and a channel that are typed push exactly one tag.
///
...
///
/// The working directory is the one the program was started in, which is a package of this
/// repository, so `git ls-remote` reads the remote of THIS checkout rather than of whatever
/// directory a shell happened to be in.
