...
///
/// The tools of this package are the Dart toolchain, YAML and the framework's own types — a
/// registry, a step, a program file — and it may know what each of them calls its own things. What
/// it may never know is any one product built on the framework: the organisation behind it, the path
/// of its repository, the words it gave its own entities.
///
/// A name here is worse than the same name in the package it judges, and that is why this file
/// exists. These audits are the thing every other package's checks are made of, so a product name
/// that lands in them travels to every package that runs them without any of them writing it down,
/// and no scan outside this repository reads these files.
///
