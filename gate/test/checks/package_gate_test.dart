...
void main() {
  ...
  test('a run that found no package is not green', () async {
    ...
    expect(
      ...
      isFalse,
      reason:
          'the line alone is not the answer the caller reads. Measured on ansiwise-cli at '
          'origin/master ddef96c, where green was failures.isEmpty and nothing else: '
          'GateVerdict(<String>[], covered: <String>[]).green answered TRUE while its own line said '
          'FAIL, so tool/ci.dart printed that refusal to stdout and returned zero',
    );
  });
...
Directory _scratch() {
  final Directory directory = Directory.systemTemp.createTempSync('gate-package-gate-');
