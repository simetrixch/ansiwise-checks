...
void main() {
  group('an ordinary tree', () {
    ...
    test('answers, in the tree this suite runs in, with the package this suite is in', () {
      // The name is read out of the manifest beside this suite rather than written here, because
      // this file is one text wherever the gate is run from and a name written in would be true in
      // exactly one repository.
      final String? here = declaredPackageName(File('pubspec.yaml').readAsStringSync());
      expect(here, isNotNull, reason: 'a suite running outside a package proves nothing about one');
      expect(
