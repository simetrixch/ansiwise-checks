...
///
/// A repository's `tool/analysis.dart` judges the package it is compiled into, so it is a program
/// rather than a test: a test would be compiled by the analysis it is meant to fail on, and the day
/// the package stops analysing the check meant to say so is the thing that did not compile.
///
...
void main() {
  group('what the check makes of an answer', () {
    ...
    test('a clean reading says what it proved and about how many packages', () {
      ...
      expect(
        ...
        isNot(contains('--')),
        reason:
            'this reading is handed a toolchain and never sees how it starts a tool, so a flag '
            'named here is a claim about an invocation it cannot observe — take --fatal-infos out '
            'of analyzerArgv and a line spelling it goes on saying so over a tree it no longer '
            'judged',
      );
    });
    ...
    test('a package nobody looked at keeps the reading from being green', () {
      const AnalysisReading reading = AnalysisReading(
        findings: <AnalysisFinding>[],
        analysed: <String>['planted_resolved'],
        notAnalysed: <String>['planted_unresolved'],
      );
      expect(
        reading.green,
        isFalse,
        reason:
            'the gap is named on the verdict line either way, and a run that named it and then '
            'answered OK would be a check reporting that it did not look, in the shape of a pass',
      );
      expect(reading.verdictLine, contains('NOT ANALYSED'));
      expect(reading.verdictLine, contains('planted_unresolved'));
    });
  });
  ...
  group('counter-probe: the real tools still go red', () {
    ...
    setUpAll(() {
      broken = Directory.systemTemp.createTempSync('gate-analysis-broken-');
      clean = Directory.systemTemp.createTempSync('gate-analysis-clean-');
    ...
    test('THE PLANTED RESOLUTION: the formatter answers one file two ways', () async {
      ...
      expect(
        formatterChangesIn(
          await const RealDartToolchain().format(directory: subject),
          package: 'planted_subject',
        ),
        isNotEmpty,
        reason:
  ...
  group('the invocation, which the probes above do not reach', () {
    ...
    setUpAll(() {
      onlyAnInfo = Directory.systemTemp.createTempSync('gate-info-');
...
Directory _scratch(String name) {
  final Directory directory = Directory.systemTemp.createTempSync('gate-analysis-$name-');
