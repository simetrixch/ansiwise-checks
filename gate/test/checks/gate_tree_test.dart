...
void main() {
  group('the path arithmetic tool/ does without package:path', () {
    test('a program under tool/ finds the package it is part of', () {
      expect(
        packageOfToolScript(Uri.file('/repos/a-repository/tool/ci.dart')).path,
    ...
    test('the last segment is found whichever separator wrote the path', () {
      expect(baseName(r'D:\repos\a-repository\tool'), 'tool');
      expect(baseName('/work/a-repository/tool'), 'tool');
    });
...
Directory _scratch() {
  final Directory directory = Directory.systemTemp.createTempSync('gate-tool-tree-');
