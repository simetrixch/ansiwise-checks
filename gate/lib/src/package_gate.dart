...
final class GateVerdict {
  ...
  /// Whether everything passed AND anything was looked at.
  ///
  /// A run that covered no package collected no failure for the same reason a clean run collects
  /// none, and only one of the two is a pass. Without the second half the verdict below answers
  /// `no Dart package was found` and this getter still says green, so the caller prints a refusal
  /// and exits zero.
  bool get green => failures.isEmpty && covered.isNotEmpty;
